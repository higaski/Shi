...
int mp_count_primes_by_trial_division() {
  ...
  mp_obj_t count_primes_by_trial_division_bytecode =
      mp_compilestring("n = count_primes(100000)", MP_PARSE_FILE_INPUT);

  START_CYC_CNT();
  mp_dobytecode(count_primes_by_trial_division_bytecode);
  STOP_CYC_CNT();

  mp_dostring("primes_module.primes_result(n)", MP_PARSE_FILE_INPUT);
